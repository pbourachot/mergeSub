    ...
    def merge(self, l1, l2):
        ...
        while (len(ll1) > 0 and len(ll2) >0):
            if (e1.start ) < (e2.start):
                if (e1.stop < e2.start):
                    list.append(MergedPart(index, e1.start, e1.stop, "E1 " + e1.text))
                    e1 = ll1.pop()
                else :
                    list.append(MergedPart(index, e1.start, max(e1.stop, e2.stop), "[" + e1.text +"][" + e2.text + "]"))
                    e1 = ll1.pop()
                    e2 = ll2.pop()
            else :
                if (e2.stop < e1.start):
                    list.append(MergedPart(index, e2.start, e2.stop, "E2 " +e2.text))
                    e2 = ll2.pop()
                else :
                    list.append(MergedPart(index, e2.start, max(e1.stop, e2.stop), "[" + e1.text +"][" + e2.text + "]"))
                    e1 = ll1.pop()
                    e2 = ll2.pop()
        ...
        return SubPartList(list, "Merged")
